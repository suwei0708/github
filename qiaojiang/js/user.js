$(function() {
    ...
    $('.user-collect .close-history').on('click', function() {
        ...
        }else if($(this).attr("myattr")=="article"){
            if(confirm("您是否删除该文章收藏")){
                $.post("/Userinfo/Delarticle/",{id:$(this).attr("myid")},function(){
                })
                $(this).parents('li').remove();
            }
        }
        ...
        $.each($('.user-data .txt'), function(index) {
            ...
            if(_this.attr("id") != "city-input" && !$.trim(_this.val())) {
                if(_this.nextAll('.tips').size()) {
                    ...
                    _this.nextAll('.tips').find('.text').text(_this.parents('dl').find('dt').text().replace('*', '') + '不能为空')
                }
                else {
                    _this.after('<div class="tips tips-error"><span class="user-ico"></span><span class="text">' + _this.parents('dl').find('dt').text().replace('*', '') + '不能为空</span></div>')
                }
            }
        });
    ...
    $('.user-data .txt').on('blur', function() {
        ...
        if(!$.trim(_this.val()) && !_this.hasClass('sex')) {
            if(_this.nextAll('.tips').size()) {
                _this.nextAll('.tips').attr('class', 'tips tips-error');
                _this.nextAll('.tips').find('.text').text(_this.parents('dl').find('dt').text().replace(/[*]/g, '') + '不能为空')
            }
