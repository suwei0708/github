...
var timeID;
    ...
    function qj_reset(){
        ...
        player= videojs('my-video', options, function() {
            ...
            _player.on('timeupdate', function(e) {  //播放时间改变执行函
                ...
                if(parseInt(qj_time%50)==0){
                    $.post("/Video/Setvideotime",{vid:vid,t:escape(_player.currentTime()),num:now},function(data){
                    })
            ...
            _player.on('play', function(e) {  //播放时间改变执行函
                if($("#nologin").length){
                  ...
                  $.post("/Video/Lookvideo/Id/0",{vid:vid},function(data){
    ...
    function playerInitVideo() {
        ...
        $('.vjs-control-bar').append('<div class="fl"></div><div class="fr"></div>');
        ...
        $('.vjs-control-bar .fl').append($('.vjs-play-control'));
        $('.vjs-control-bar .fl').append($('.vjs-remaining-time'));
        // 增加高清切换
