function Tag(inputId) {
    ...
    obj.initView = function() {
        ...
        appendStr += '<input type="text" class="tag-input" placeholder="添加标签" maxlength="8"/><div class="tagList"></div>';
        appendStr += '</div>';
        inputObj.after(appendStr);
        var inputValue = inputObj.val();
        tagTake.setInputValue(inputId, inputValue);
        var tagInput = jQuery("#" + inputId + "_tagcontaine .tag-input");
        if (!this.isDisable) {
            ...
            $('.tag-tips').on('click', 'a', function() {
                var inputValue2 = jQuery(this).html();
                var arr = jQuery('#tagValue').val().split(',');
                if(arr.length == 8) {
                    jQuery(this).val('');
                    alert('最多只能8个标签');
                ...
                for (var i = 0; i < arr.length; i++) {
                    if(arr[i] == inputValue2) {
                    jQuery(this).val('');
                        alert('不能输入相同标签');
                        return false;
                    }
                }
                tagTake.setInputValue(inputId, inputValue2);
                jQuery(this).val('');
                return false;
            })
        } else {
...
var tagTake = {
    ...
    "getTagItemModel": function(valueStr) {
        return '<div class="tagItem"><span>' + valueStr + '</span><div class="icon-close qjfont qj-cha"></div></div>';
    }
}