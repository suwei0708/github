$(function() {
    ...
    function goscrollTop() {
        ...
        if ($('.body-fixed').length && location.href.indexOf("UserVideo") <= 0) {
    ...
    if ($('.box-upload').length) {
        ...
        $('.u-select-list').on('click', 'li', function() {
            ...
            $(this).parents('.u-select').find('.input-val').val($(this).attr("myid"));
        });
    ...
    // 视频管理删除
    $('.box-gl-ct').on('click', '#ico-delete', function() {
        $('.popup-del').show();
        mask.show();
    });
    ...
    // 上传视频
    $('.box-gl-ct').on('click', '#btn-add', function() {
        $('.popup-video').show();
        mask.show();
        return false;
    });
    ...
    $('.v-tab').on('click', 'li', function() {
        ...
        if (urls.indexOf("/video/") > 0) {
            $(this).addClass('cur').siblings().removeClass('cur');
            if ($(this).index() != 1) {
                if ($(this).index() == 2) {
                    $('.v-tab-cont').hide().eq(2).show();
                } else {
                    $('.v-tab-cont').hide().eq(0).show();
                    $("#istype").val($(this).index())
                    mypage(1);
                }
            } else {
                $('.v-tab-cont').hide().eq(1).show();
            }
        } else if (urls.indexOf("UserMessage/") <= 0) {
            $(this).addClass('cur').siblings().removeClass('cur');
    ...
    $('.v-comment').on('click', '.reply', function() {
        var _this = $(this);
        if (_this.parents('.com-box').text().indexOf("回 复") <= 0) {
            _this.parents('.func').after($("#tmp").html());
        }
        if ($("#myproid").length) {
            $("#myproid").val(_this.attr("myproid"));
        }
        var comment = _this.parents('.func').nextAll('.input2');
    ...
    $("#bsearch").click(function() {
        ...
        var type = $(".search-select-val").html();
        if (val != "") {
            if (type == '视频') {
                ...
                //$.get("/Video/Getpy/?key=" + encodeURI(val), function(data) {
                $("#searchform").attr("action", "/article/?key=" + encodeURI(val));
                $("#searchform").submit();
                //})
            }
        }
    ...
    $(".xiazai").click(function() {
        ...
        var vids = $(".play-list").find(".cur").attr("myid");
        $(this).attr("href", '/Index/Down/Id/' + vid + '?vid=' + vids);
    })

    var weburl = location.href;
    if (weburl == "http://" + location.host + "/") {
        $(".header li").eq(0).addClass("cur");
    } else {
        $(".header li").each(function() {
            var _this = $(this);
            if (weburl.indexOf(_this.find("a").attr("href")) > 0 && _this.text() != "首页") {
                _this.addClass("cur");
            }
        })
    }
...
function loginLine() {
    if (jQuery('.popup-play').find('.nav_line').length) {
        return false
    };
...
$(".qj-shanchu").click(function() {
    $.get("/Other/Delsearchkeys", function(result) {
        $(".jilu").html("");
    });
})

$("#qj_login").on("click", function() {
    $.get("/Userlogin/Islogin", function(data) {
        $("#islogin").html(data);
    })
})

$("#qj_reg").on("click", function() {
    $.get("/Userlogin/Islogin", function(data) {
        $("#islogin").html(data);
